...
void JRenderer::RenderQuad(JQuad* quad, float xo, float yo, float angle, float xScale, float yScale)
{
	...
	vfpu_sincos(angle, &sinAngle, &cosAngle);
#else
	float cosAngle, sinAngle;
	cosAngle = cosf(angle);
	sinAngle = sinf(angle);
...
void JRenderer::SetFOV(float fov)
{
	...
	sceGumMatrixMode(GU_PROJECTION);
	sceGumLoadIdentity();
	sceGumPerspective(mFOV,480.0f/272.0f,0.5f,1000.0f);
}
...
void JRenderer::DrawRoundRect( float x1,float y1, float w,float h,float radius,PIXEL_TYPE color )
{
	...
	for(int i=0;i<y2-y1; i++)
	{
		...
		{
#ifdef PSP
			q=vfpu_sqrtf(radius*radius - (radius-i)*(radius-i));
#else
		...
		else if (i > y2-y1 - radius)
		{
			//(i-(y2-y1-radius))
#ifdef PSP
			q=vfpu_sqrtf(radius*radius - (i-(y2-y1-radius))*(i-(y2-y1-radius)));
#else

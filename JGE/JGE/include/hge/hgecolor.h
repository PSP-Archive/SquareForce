...
public:
	...
	hgeColorRGB(const float& _r, const float& _g, const float& _b, const float& _a) { r=_r; g=_g; b=_b; a=_a; }
	hgeColorRGB(const DWORD& col) { SetHWColor(col); }
	...
	void			SetHWColor(const DWORD& col) {	a = (col>>24)/255.0f; r = ((col>>16) & 0xFF)/255.0f; g = ((col>>8) & 0xFF)/255.0f; b = (col & 0xFF)/255.0f;	}
...
public:
	...
	hgeColorHSV(const float& _h, const float& _s, const float& _v, const float& _a) { h=_h; s=_s; v=_v; a=_a; }
	hgeColorHSV(const DWORD& col) { SetHWColor(col); }
	...
	void			SetHWColor(const DWORD& col);

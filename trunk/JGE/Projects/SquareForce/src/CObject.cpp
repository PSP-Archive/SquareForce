...
void CObject::RenderWire(const Vector2D& camPos, const float& camRot, const Vector2D& pos, PIXEL_TYPE color)
{
	...
	b2Vec2 trans = popCast(b2Vec2, pos) - mBody->GetOriginPosition();
		...
		case e_polyShape:
			{
				...
				{
					b2Mat22 rot(camRot);
					b2Vec2 v = b2MulT(rot, trans + poly->m_position - popCast(b2Vec2, camPos) + b2Mul(poly->m_R, poly->m_vertices[i]));
